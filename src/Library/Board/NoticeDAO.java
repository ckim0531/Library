...
public class NoticeDAO {
	...
	// 게시판 글 생성
	public void noticeBoardInsert(NoticeBean bb) {
		int num = 0;
		...
		try {
			...
			// sql 쿼리 작성
			// 게시판 글 번호 num 구하기
			sql = "select max(num) from board_notice";
	...
	// 게시판 전체 글 개수 가져오기
	public int noticeBoardGetCount() {
		int count = 0;
	...
	// 게시판 전체 글 가져오기
	public List noticeBoardGetList() {
		...
		try {
			...
			sql = "select * from board_notice order by re_ref desc, re_seq asc";
	...
	// 게시판 페이지 나타내기
	public List noticeBoardGetList(int startRow, int pageSize) {
		List boardList = new ArrayList();
		...
		try {
			...
			sql = "select * from board_notice order by re_ref desc,re_seq asc limit ?, ?";
	...
	// 조회수 증가
	public void noticeBoardUpdateReadcount(int num) {
		try {
			// 드라이버 로드, 디비 연결
			con = getConnect();
			...
			sql = "update board_notice set readcount = readcount + 1 where num = ?";
	...
	// 글 번호에 해당하는 게시글 가져오기
	public NoticeBean noticeBoardGetContentNum(int num) {
		NoticeBean bb = null;
		...
		try {
			...
			sql = "select * from board_notice where num = ?";
	...
	// 게시글 수정하기
	public int noticeBoardUpdate(NoticeBean bb) {
		int check = -1;
		...
		try {
			...
			// 글 번호에 해당하는 아이디가 있으면
			sql = "select id from board_notice where num = ?";
			...
			if(rs.next()) {
				...
				if(bb.getId().equals(rs.getString("id"))) {
					...
					sql = "update board_notice set subject = ?, content = ?, file = ?"
							+ "where num = ?";
	...
	public String noticeGetFileName(int num) {
		...
		try {
			...
			// 게시판에 글이 있는지 확인
			sql = "select file from board_notice where num = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
	...
	// 게시글 업로드한 파일 삭제
	public int noticeDeleteFile(int num, String id) {
		int check = -1;
		...
		try {
			...
			if (rs.next()) {
				...
				if(id.equals(rs.getString("id"))) {
					// 삭제 처리 쿼리 구문
					sql = "update board_notice set file = '' where num = ?";
	...
	// 게시판 글 삭제
	public int noticeDelete(int num, String id) {
		int check = -1;
		...
		try {
			...
			if (rs.next()) {
				...
				if(id.equals(rs.getString("id"))) {
					// 삭제 처리 쿼리 구문
					sql = "delete from board_notice where num = ?";
	...
	// 게시판 답 글 쓰기
	public void board_noticeReInsert(NoticeBean bb) {
		int num = 0;
		...
		try {
			...
			sql = "update board_notice set re_seq=re_seq+1 where re_ref=? and re_seq>?";
			pstmt = con.prepareStatement(sql);
