...
<body>
	<%
		...
		bb.setBook_num(Integer.parseInt(multi.getParameter("num"))); // 책 번호
		bb.setBook_name(multi.getParameter("name")); // 책 이름
		bb.setBook_author(multi.getParameter("author")); // 저자
		bb.setBook_count(Integer.parseInt(multi.getParameter("count"))); // 책 수량
		// 책 들어온 날짜
		
		// 전송받은 데이터가 파일일 경우 getFilesystemName()으로 파일 이름을 받아올 수 있다.
		bb.setBook_image(multi.getFilesystemName("image")); // 책 표지
		...
		while (e.hasMoreElements()) {
			...
			System.out.println("클라이언트 -> 서버 업로드 파일의 원본 이름:" + multi.getOriginalFileName(image) + "<br>");
			System.out.println("서버에 실제로 업로드된 파일의 이름 :" + multi.getFilesystemName(image) + "<br>");
			...
			bdao.bookBoardInsert(bb);
			
			int insert_num=bdao.bookGetBoardNum();
		
			response.sendRedirect("bookView.jsp?num="+insert_num+"&pageNum="+pageNum);
		}
	%>
</body>
</html>