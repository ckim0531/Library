...
<%
	// 디비처리 객체 생성 BoardDAO
	pressDAO ndao = new pressDAO();

	// getBoardCount() 생성후 처리 
	int count = ndao.pressBoardGetCount();
	...
	if (count != 0) {
		...
		boardList = ndao.pressBoardGetList(startRow, pageSize);
	}
%>
...
<div id="container">
	<div>
		...
		<div class="conts">
			...
			<p class="summary">도서관 보도자료 입니다.</p>
			<form id="form" name="form" method="post" action="../../jsp/press/pressList.jsp">
					...
					<%
						if(count != 0) {
							for (int i = 0; i < boardList.size(); i++) {
								...
								pressBean nb = (pressBean) boardList.get(i);
						%>
						<tbody>
							<tr>
								<td><%=nb.getNum()%></td>
								<td class="al l20">
									<%
									if(nb.getFile().equals("")) {
									%>
									<a href="pressView.jsp?num=<%=nb.getNum()%>&pageNum=<%=pageNum%>">
										<%=nb.getSubject()%></a>
									<%
									}
									else {
			...
			%>
			<div class="ar mt10">
				<a href="pressWrite.jsp?pageNum=<%=pageNum%>" class="btn03 type2 type3">글쓰기</a>
			</div>
			<%
					}
				}
			...
			<div class="page">
				<ul>
					...
					%>
					<a href="pressList.jsp?pageNum=<%=startPage - pageBlock%>">
					...
					%>
					<li class="on">
						<a href="pressList.jsp?pageNum=<%=i%>"><%=i%></a>
					<%
								}
								else
								{
