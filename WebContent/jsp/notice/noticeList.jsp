...
<%
	...
	int count = ndao.noticeBoardGetCount();
	...
	if (count != 0) {
		...
		boardList = ndao.noticeBoardGetList(startRow, pageSize);
	}
%>
...
<div id="container">
	<div>
		...
		<div class="conts">
			...
			<form id="form" name="form" method="post" action="../../jsp/notice/noticeList.jsp">
					...
					<%
						if(count != 0) {
							for (int i = 0; i < boardList.size(); i++) {
								...
								NoticeBean nb = (NoticeBean) boardList.get(i);
						%>
						<tbody>
							<tr>
								<td><%=nb.getNum()%></td>
								<td class="al l20">
									<%
									System.out.println("파일 제목 : " + nb.getFile());
									if("".equals(nb.getFile())) {
										System.out.println("'null이나 공백이면' : " + nb.getFile());
									%>
										<a href="noticeView.jsp?num=<%=nb.getNum()%>&pageNum=<%=pageNum%>">
										<%=nb.getSubject()%></a>
									<%
									}
									else if(!"".equals(nb.getFile())) {
										System.out.println("'null이나 공백이 아니면' : " + nb.getFile());
									%>
			...
			%>
			<div class="ar mt10">
				<a href="noticeWrite.jsp?pageNum=<%=pageNum%>" class="btn03 type2 type3">글쓰기</a>
			</div>
			<%
					}
				}
			...
			<div class="page">
				<ul>
					...
					%>
					<a href="noticeList.jsp?pageNum=<%=startPage - pageBlock%>">
